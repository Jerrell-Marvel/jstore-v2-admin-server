...
export const addProductVariantImages = async (images: Express.Multer.File[], productId: number, client?: PoolClient) => {
  const imageValues = Object.values(images).map((image) => [image.filename, productId]);

  const query = format(`INSERT INTO variant_images (image_url, variant_id) VALUES %L RETURNING product_image_id`, imageValues);
